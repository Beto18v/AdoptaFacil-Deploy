...
interface RefugiosPageProps {
    shelters: Shelter[];
    selectedCity?: string | null;
}

export default function Refugios() {
    const { shelters, selectedCity } = usePage().props as unknown as RefugiosPageProps;
            ...
            <main className="flex-1">
                <div className="relative border-t border-blue-200/50 bg-gradient-to-br from-blue-100/80 via-green-100/60 to-blue-200/40 py-12 md:py-16 dark:border-blue-800/30 dark:from-blue-950/40 dark:via-green-950/30 dark:to-blue-900/50">
                    ...
                    <div className="relative container mx-auto px-4 md:px-6">
                        {selectedCity && (
                            <div className="mb-6 rounded-2xl border border-blue-200/60 bg-white/80 px-5 py-4 text-sm font-medium text-gray-700 shadow-lg backdrop-blur-sm dark:border-blue-800/50 dark:bg-gray-800/80 dark:text-gray-200">
                                Mostrando refugios registrados en <span className="font-bold text-blue-700 dark:text-blue-300">{selectedCity}</span>.
                            </div>
                        )}
                        ...
                        {shelters && shelters.length > 0 ? (
                            ...
                            <div className="mt-16 text-center text-gray-500 dark:text-gray-400">
                                <h2 className="text-2xl font-semibold">
                                    {selectedCity ? `No hay refugios registrados en ${selectedCity}` : 'Aun no hay refugios'}
                                </h2>
                                <p className="mt-2">
                                    {selectedCity
                                        ? 'Prueba con otra ciudad o vuelve mas tarde para revisar nuevos registros.'
                                        : 'Vuelve pronto para ver las fundaciones aliadas a nuestra causa.'}
                                </p>
                            </div>
                        )}
                    </div>
